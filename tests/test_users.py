...
def test_admin_can_create_and_disable_user(client, app):
    ...
    create = client.post(
        "/admin/users",
        data={"email": "operator-new@example.com", "role": "Operator", "password": "Str0ngPass!2026"},
    ...
    login = client.post(
        "/login",
        data={"email": "operator-new@example.com", "password": "Str0ngPass!2026"},
        follow_redirects=False,
    )
...
def test_admin_can_change_role_and_reset_password(client, app):
    ...
    with app.app_context():
        ...
        user.set_password("0ldStr0ngPass!")
        db.session.add(user)
        db.session.commit()
        user_id = user.id
    ...
    with app.app_context():
        ...
        assert updated.check_password("Str0ngPass!2026")


def test_user_can_change_own_password(client, app):
    _auth_as(client, app, "self-change@example.com", "Operator", password="0ldStr0ngPass!")

    resp = client.post(
        "/account/password",
        data={
            "current_password": "0ldStr0ngPass!",
    ...
    with app.app_context():
        ...
        assert user.check_password("Str0ngPass!2026")
